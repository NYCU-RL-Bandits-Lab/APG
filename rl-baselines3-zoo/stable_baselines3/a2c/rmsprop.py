...
class RMSprop(Optimizer):
    def __init__(
        ...
        print(f'foreach: {foreach}')
        print(f'maximize: {maximize}')
        print(f'differentiable: {differentiable}')
        ...
        if nesterov:
            ...
            print('-'*70)
            print('-'*70)
            print(f'Using Nesterov with momentum {momentum}')
            print('-'*70)
            print('-'*70)
            print('-'*70)
        elif momentum > 0.0:
            print('-'*70)
            print(f'Using Heavy Ball with momentum {momentum}')
            print('-'*70)
        else:
            print('-'*70)
            print('w/o momentum')
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if momentum > 0:
            ...
            if nesterov:
                param.add_(buf.mul(momentum).addcdiv(grad, avg), alpha=-lr)
            else:
...
def _multi_tensor_rmsprop(
    ...
    *,
    ...
    for (((grouped_params, grouped_grads, grouped_square_avgs, grouped_grad_avgs,
        ...
        if momentum > 0 and step_num > 500000:
            ...
            if nesterov:
                # print('nesterov')
                tmp = copy.deepcopy(grouped_momentum_buffer_list)
                torch._foreach_mul_(tmp, momentum)
                torch._foreach_addcdiv_(tmp, grouped_grads, avg)
                torch._foreach_add_(grouped_params, tmp, alpha=-lr)
                del tmp
            else:
                # print('heavy ball')
                torch._foreach_add_(grouped_params, grouped_momentum_buffer_list, alpha=-lr)
                # print(grouped_momentum_buffer_list)
                # print([torch.norm(grouped_momentum_buffer_list[i]) for i in range(len(grouped_params))])
                # print([torch.norm(grouped_grads[i]) for i in range(len(grouped_grads))])
                # print('='*50)
                # print(torch.norm(grouped_momentum_buffer_list))
                # print(grouped_params)
                # print()
                # print(grouped_momentum_buffer_list)
            # if step_num % 5 == 0:
            #     for i in range(len(grouped_momentum_buffer_list)):
            #         grouped_momentum_buffer_list[i] = torch.zeros_like(
            #             grouped_momentum_buffer_list[i], memory_format=torch.preserve_format
            #         )
            # print([torch.norm(grouped_momentum_buffer_list[i]) for i in range(len(grouped_params))])
        else:
