...
class Bellman:
    ...
    def compute_V_Q_Adv(self, pi_t):
        ...
        # V(rho)
        V_rho_t = np.inner(self.initial_state_distribution, V_t.squeeze())
        V_rho_t = np.expand_dims(V_rho_t, axis=0)

        return V_t, Q_t, Adv_t, V_rho_t
    ...
    # -------------- compute true grad --------------
    def compute_natural_true_grad(self, pi_t, Adv_t, d_mu_t):

        # grad = [ 1 / (1 - gamma) ] * Adv  (ref to Agarwal work)
        delta_theta_t = np.zeros_like(pi_t, dtype=np.float64)
        delta_theta_t = Adv_t * (1.0 / (1.0 - self.gamma))
