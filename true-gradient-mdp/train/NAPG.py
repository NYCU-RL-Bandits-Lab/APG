...
class NAPG_model(Bellman, Saver):
    ...
    def __init__(self, args: object):
        ...
        self.save_path = os.path.join(args.log_dir, 'NAPG', f'mean.parquet')
    ...
    def learn(self, epoch: int):
        ...
        # log
        logger.info(f"[NAPG] Start Running")
        ...
        for seed in range(self.seed_num):
            ...
            for timestep in range(epoch):
                ...
                if ((timestep+1) % self.chunk_size == 0) or (timestep == 0) or (timestep in self.save_time):
                    logger.debug(f"[NAPG] Epoch {timestep:^10d}/{epoch:^10d}")
                ...
                if ((timestep+1) % self.chunk_size == 0) or (timestep == 0):
                    record = np.concatenate((
                        ...
                        V_omega_t, 
                        V_omega_rho_t, 
                        -np.log(self.V_opt - V_omega_rho_t.item()),
                        Q_t, 
                        Adv_t, 
                        d_t, 
                        d_rho_t,
                ...
                # remove restart mechanism
                if True:
                    omega_t = copy.deepcopy(phi_t)
                else:
                    omega_t = copy.deepcopy(theta_t)
                    logger.debug(f"[NAPG] {timestep}: non-monotone, V_phi_rho_t = {V_phi_rho_t.item()}, V_rho_t = {V_rho_t.item()}")

                # store theta_{t-1} for Nesterov's accelerating
                theta_t_1 = copy.deepcopy(theta_t)

        logger.info(f"[NAPG] Finish Running")