...
class APG_model(Bellman, Saver):
    ...
    def __init__(self, args: object):
        ...
        self.save_path = os.path.join(args.log_dir, 'APG', f'mean.parquet')
    ...
    def learn(self, epoch: int):
        ...
        # log
        logger.info(f"[APG] Start Running")
        ...
        for seed in range(self.seed_num):
            ...
            for timestep in range(epoch):
                ...
                if ((timestep+1) % self.chunk_size == 0) or (timestep == 0):
                    logger.debug(f"[APG] Epoch {timestep:^10d}/{epoch:^10d}")
                ...
                if monotone:
                    ...
                    logger.debug(f"[APG] {timestep}: non-monotone, V_phi_rho_t = {V_phi_rho_t.item()}, V_rho_t = {V_rho_t.item()}")

                # store theta_{t-1} for Nesterov's accelerating
                theta_t_1 = copy.deepcopy(theta_t)

        logger.info(f"[APG] Finish Running")